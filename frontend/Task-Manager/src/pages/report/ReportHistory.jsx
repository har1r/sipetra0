...
import ReportTableRow from "../../components/tabels/ReportTableRow";
import ReportFilterSection from "../../components/filters/ReportFilterSection";
import AddAttachmentModal from "../../components/input/AddReportAttachmentModal";

const ReportHistory = () => {
  const { state, actions, refs } = useManageReport();
      ...
      <div className="max-w-[1600px] mx-auto py-8 px-6 space-y-8 animate-fadeIn">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          ...
          <div className="flex items-center gap-3">
            ...
            >
              <HiOutlineRefresh
                className={`w-4 h-4 ${state.isLoading ? "animate-spin text-emerald-500" : "group-hover:rotate-180 transition-transform duration-500"}`}
              />
              Reset Filter
            </button>
            <button
        ...
        <ReportFilterSection
          ...
          setFilterDraft={actions.setFilterDraft}
        ...
        <div className="bg-white rounded-[1.5rem] border border-slate-200 overflow-hidden shadow-sm">
            ...
            <table className="w-full text-left border-collapse min-w-[1000px]">
              <thead className="bg-slate-800 text-slate-200 text-[10px] font-bold tracking-wider uppercase">
                <tr>
                  ...
                  <th className="px-4 py-5">Identitas Berkas</th>
                ...
                {state.isLoading ? (
                  ...
                  state.tasks.map((task) => (
                    <ReportTableRow
                      ...
                      onAddLink={() => actions.openAttachmentModal(task)}
                    />
                  ))
                ) : (
                  <tr>
          ...
          <div className="p-8 border-t border-slate-100 flex flex-col sm:flex-row justify-between items-center bg-slate-50/30 gap-6">
            <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest bg-white px-5 py-2.5 rounded-xl border border-slate-200 shadow-sm">
              Total: {state.pagination.totalData} Berkas
            </span>
            <Pagination
        ...
        <div className="space-y-6 pt-10">
          ...
          <div className="bg-white rounded-[1.5rem] p-2 border border-slate-200 shadow-sm overflow-hidden">
            <ReportHistoryTable ref={refs.historyTableRef} />
          </div>
        </div>

        <AddAttachmentModal
          isOpen={state.attachmentForm.isOpen}
          formData={state.attachmentForm}
          onUpdateField={actions.updateAttachmentField}
          onClose={actions.closeAttachmentModal}
          onSubmit={actions.submitAttachment}
        />
      </div>
    </DashboardLayout>
  );
...
// import {
//   HiOutlineSearch, HiOutlineRefresh, HiOutlineInbox, HiOutlineLink,
//   HiOutlineExternalLink, HiFilter, HiOutlinePrinter
// } from "react-icons/hi";
// import { FaSquareCheck, FaRegSquare } from "react-icons/fa6";
...
//       if (!pdfWindow) {
//         toast.error("Gagal membuka tab baru. Izinkan popup di browser Anda.", { id: toastId });
//       } else {
...
//       } else {
//         toast.error(err.message || "Gagal mencetak Kertas Kerja.", { id: toastId });
//       }
//     }
//   };
...
//     try {
//       const response = await axiosInstance.post(API_PATHS.REPORTS.CREATE_REPORT, {
//         selectedTaskIds: selectedIds
//       });

//       toast.success(response.data.message || "Batch pengantar berhasil dibuat.", { id: toastId });
//       setSelectedIds([]);
//       fetchTasks();
//       if (historyTableRef.current?.refresh) historyTableRef.current.refresh();
//     } catch (err) {
//       toast.error(err.response?.data?.message || "Gagal membuat batch.", { id: toastId });
//     } finally {
//       setExporting(false);
//     }
//   };
...
//             </h1>
//             <p className="text-slate-400 text-xs font-semibold mt-1 tracking-wide uppercase">
//               Kumpulkan permohonan tervalidasi ke dalam satu surat pengantar kolektif.
//             </p>
//           </div>
...
//               className="group flex items-center gap-2 px-6 py-3 bg-white border border-slate-200 rounded-2xl text-[11px] font-bold text-slate-600 shadow-sm hover:border-emerald-500 transition-all active:scale-95"
//             >
//               <HiOutlineRefresh className={`w-4 h-4 ${isLoading ? "animate-spin text-emerald-500" : "group-hover:rotate-180 transition-transform duration-500"}`} />
//               Reset Filter
//             </button>
//             <button
...
//               onChange={(e) => setFilterDraft({ ...filterDraft, nopel: e.target.value })}
//             />
//             <FilterInput
//               label="Mulai Tanggal"
//               type="date"
...
//             />
//             <div className="group space-y-1.5">
//               <label className="text-[10px] font-bold text-slate-500 ml-1 tracking-widest">Urutan</label>
//               <select
//                 value={filterDraft.sortOrder}
//                 onChange={(e) => setFilterDraft({ ...filterDraft, sortOrder: e.target.value })}
...
//             className="w-full lg:w-auto px-8 py-2.5 bg-emerald-500 hover:bg-emerald-600 text-white rounded-xl text-[11px] font-bold shadow-lg transition-all flex items-center justify-center gap-2"
//           >
...
//                 <tr>
//                   <th className="px-6 py-5 text-center w-16">
//                     <button onClick={toggleSelectAll} className="text-lg transition-all active:scale-90">
...
//                     <tr key={i} className="animate-pulse">
//                       <td colSpan="6" className="px-6 py-8"><div className="h-5 bg-slate-100 rounded-full w-full" /></td>
//                     </tr>
//                   ))
//                 ) : tasks.length > 0 ? (
//                   tasks.map((task) => (
//                     <tr
//                       key={task._id}
//                       className={`hover:bg-slate-50/80 transition-all group ${selectedIds.includes(task._id) ? "bg-emerald-50/30" : ""}`}
//                       onClick={() => toggleSelectOne(task._id)}
//                     >
//                       <td className="px-6 py-4 text-center" onClick={(e) => e.stopPropagation()}>
//                         <button onClick={() => toggleSelectOne(task._id)} className="text-lg">
...
//                           <span className="text-[13px] font-black text-slate-700 leading-none">{task.mainData?.nopel}</span>
//                           <span className="text-[10px] text-slate-400 font-mono mt-1">{task.mainData?.nop}</span>
//                         </div>
//                       </td>
//                       <td className="px-4 py-4">
//                         <div className="flex flex-col gap-1.5">
//                           <span className="text-[11px] font-bold text-slate-700 uppercase">{task.additionalData?.[0]?.newName || "TANPA NAMA"}</span>
//                           <span className="inline-flex w-fit px-2 py-0.5 bg-emerald-50 text-emerald-600 border border-emerald-100 rounded text-[9px] font-black uppercase">
//                             {task.title}
//                           </span>
//                         </div>
//                       </td>
...
//                           </span>
//                         ) : (
//                           <span className="text-[10px] font-bold text-slate-300 uppercase tracking-widest italic">Antrean</span>
//                         )}
//                       </td>
//                       <td className="px-4 py-4 text-center text-[11px] font-bold text-slate-600">{formatDateId(task.updatedAt)}</td>
//                       <td className="px-4 py-4 text-center" onClick={(e) => e.stopPropagation()}>
//                         <div className="flex justify-center items-center gap-2">
//                           {task.title === "Mutasi Sebagian" && (
//                             <button
//                               onClick={() => handleDownloadPartialMutation(task._id)}
//                               className="p-2 rounded-lg bg-emerald-50 text-emerald-600 border border-emerald-200 hover:bg-emerald-500 hover:text-white transition-all"
//                             >
//                               <HiOutlinePrinter size={14} />
//                             </button>
//                           )}
//                           <button className="p-2 rounded-lg bg-slate-50 text-slate-400 border border-slate-200 hover:text-emerald-500 hover:border-emerald-500 transition-all">
...
//                           <h3 className="text-base font-black text-slate-600 uppercase tracking-widest">Tidak Ada Data</h3>
//                           <p className="text-sm text-slate-400 font-medium">Coba sesuaikan filter pencarian Anda.</p>
//                         </div>
//                       </div>
//                     </td>
//                   </tr>
...
//           <div className="flex items-center gap-3">
//             <div className="h-6 w-1 bg-emerald-500 rounded-full" />
//             <h2 className="text-xl font-bold text-slate-800 tracking-tight">Riwayat Batch PDF</h2>
//           </div>
//           <div className="bg-white rounded-[1.5rem] p-2 border border-slate-200 shadow-sm overflow-hidden">
